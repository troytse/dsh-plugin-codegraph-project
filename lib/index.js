...
export function apply(ctx, config) {
  ...
  function mountRefusal(session, current) {
    ...
    if (session.state === 'home-directory') {
      return [
        `CodeGraph is not enabled for ${workspace}: it is the home directory, which is never treated as a project.`,
        `An index of the home directory would be claimed by every session under it, so an index there is ignored even when one exists.`,
        `Ask the user to index the specific project instead:`,
        `  npx -y @colbymchenry/codegraph@${current.version} init -y -- <project root>`,
        `If they really do want the home directory served, they can set allowHomeProject: true in this plugin's configuration.`,
      ].join('\n')
    }
    return [
